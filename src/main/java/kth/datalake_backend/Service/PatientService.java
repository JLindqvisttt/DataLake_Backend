...
public class PatientService {
    ...
    public ResponseEntity<?> loadSymptoms(MultipartFile file, String name) throws IOException {
        ...
        List<Patient> patientList = patientRepository.findAllByDataset(name);
        System.out.println("row 256: "+patientList.get(0));
        System.out.println("row 257: "+patientRepository.findPatientBySubjectId(12727));
        List<Integer> patientId = new ArrayList<>();

        ArrayList<Symptoms> symptomsList = symptomsRepository.findAll();
        if (patientList.isEmpty())
            return ResponseEntity.badRequest().body(new MessageResponse("There are no patients admitted, you must admit patients first before you can add symptoms."));
                ...
                if (id != Integer.MAX_VALUE && id != Integer.parseInt(row.getCell(rowNumbers.get("id")).toString().replace(".0", ""))) {
                    for (Patient a : patientList)
                        if (a.getSubjectId() == id){
                            //System.out.print("row 269: Patient being saved: ");
    ...
    private HashMap<String, Integer> setRowNumbers(XSSFSheet worksheet, int index) {
        ...
        for (Cell r : row) {
            switch (r.toString()) {
                ...
                case "OS_TIME" ->
                        rowNumbers.put("overall survival time", r.getColumnIndex());//overall survival time (months)
                ...
                case "STATUS", "DTH" ->
                        rowNumbers.put("overall survival status", r.getColumnIndex());//overall survival status
